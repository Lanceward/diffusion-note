...
if __name__=='__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for img, label in train_loader:
            ...
            ts = torch.randint(1, high=T+1, size=(args.b,)).to(args.device)
            eps = torch.normal(mean=torch.zeros(args.b, 1, 28, 28), std=torch.ones(args.b, 1, 28, 28)).to(args.device)
            print(ts.shape, eps.shape)
            
            ahats = alpha_hat[ts]
            img_noised = torch.sqrt(ahats).view(-1, 1, 1, 1) * img + torch.sqrt(1-ahats).view(-1, 1, 1, 1) * eps
            
            print(img_noised.shape, ts.shape, label.shape)
            
            noises = net.forward(sample=img_noised, timestep=ts, class_labels=label)
            exit()
